...
const Orders = () => {
    ...
    const { currency, axios, providerToken } = useAppContext();
    const [orders, setOrders] = useState([]);

    const fetchOrders = async () => {
        try {
            const { data } = await axios.get('/api/order/provider', {
            ...
            <div className="md:p-10 p-4 space-y-4">
                ...
                {orders.map((order, index) => (
                    <div key={index} className="flex flex-col md:items-center  md:flex-row justify-between gap-5 p-5 max-w-4xl rounded-md border border-gray-300 ">
                        ...
                        <p className="flex flex-col text-lg text-primary">{currency}{order.amount}</p>
